...
class Renderer:
    def __init__(self, face_topo_file: str):
        ...
        verts, faces_idx = load_ply(face_topo_file)
        self.verts_rgb = (
            th.ones_like(verts)[None] * th.Tensor([0.78, 0.78, 0.78])[None, None, :]
        )  # (1, V, 3)
        self.verts_rgb = self.verts_rgb.to(self.device)
        # self.faces = faces_idx.verts_idx.to(self.device)[None, :, :]
        self.faces = faces_idx.to(self.device)[None, :, :]
    ...
    def render(self, verts: th.Tensor):
        ...
        focal = th.tensor([36.0], dtype=th.float32).to(self.device)
        princpt = th.tensor([0.1, 0.1], dtype=th.float32).to(self.device).unsqueeze(0)

        cameras = PerspectiveCameras(
            device=self.device, focal_length=focal, R=R, T=T, principal_point=princpt
        )
    ...
    def to_video(
        ...
        images = th.cat(
            [self.render(v).cpu() for v in th.split(verts, batch_size)], dim=0
        )
        images = 255 * images[:, :, :, :3].contiguous().numpy()
        images = images.astype(np.uint8)

        video_stream = ffmpeg.input(
            "pipe:", format="rawvideo", pix_fmt="rgb24", s="480x640", r=fps
        )
        ...
        proc = (
            ffmpeg.output(*streams, video_output, **output_args)
            .overwrite_output()
