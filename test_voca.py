...
import pdb
import torch
from render.ply_renderer import Renderer
from model.voca import Mymodel
import numpy as np
...
wav_path = './audio/voca.wav'
template_path = './template/FaceTalk_170809_00138_TA.ply'

model = Mymodel()
model.load_state_dict(torch.load("./checkpoints/voca.pth"))
model = model.cuda()
model.eval()
...
index = pickle.load(open("./template/FLAME_masks.pkl", "rb"), encoding="latin1")
rigid_index = index["boundary"]
neck_index = index["neck"]

repeat_index = []
for i in range(rigid_index.shape[0]):
    for j in range(neck_index.shape[0]):
...
if audio.ndim != 1:
    ...
    audio = audio[:, 0]
proc_audio = process_audio('./ds_graph/output_graph.pb', audio, sample_rate, multiface=False)[::2]
...
template_verts, f = load_ply(template_path)
...
with torch.no_grad():
    ...
    offset = expression_offset[:, :-1, :] / 1000
...
for i in range(x.shape[0]):
    ...
    rot_vert = np.zeros((5023, 3))
    for j in range(0, 5023):
        if j == rigid_index[num]:
            num = np.minimum(num + 1, 113)
            rot_vert[j] = np.dot(rot1, x[i, j])
        elif j == neck_index[neck_num]:
            neck_num = np.minimum(neck_num + 1, 200)
            rot_vert[j] = np.dot(rot2, x[i, j])
        else:
            rot_vert[j] = np.dot(rot, x[i, j])
    out.append(rot_vert)

out = torch.from_numpy(np.stack(out, axis=0)).to(torch.float32)


render.to_video(
    out,
    audio_file=wav_path,
    video_output="./voca.mp4",
    fps=30,
    batch_size=30,
)