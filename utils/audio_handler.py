        ...
        with tf.compat.v1.Session(graph=graph) as sess:
            for subj in audio.keys():
                for seq in audio[subj].keys():
                    ...
                    if mutiface:
                        num_frames0 = int(round(audio_len_s * 30))
                        network_output = interpolate_features(network_output[:, 0], 50, 30,
                                                            output_len=num_frames0)
                    else:
                        num_frames0 = int(round(audio_len_s * 60))
